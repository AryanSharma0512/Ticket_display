...
class PDF extends FPDF {
    ...
    function Footer() {
        ...
        $this->Cell(0, 10, 'Page ' . $this->PageNo() . ' | © Shree Dhanlaxmi Travels 2025.', 0, 0, 'C');
    }
}
...
foreach ($transactions as $row) {
    ...
    $pdf->Cell($widths[1], 8, $row['transaction_id'], 1);
    $pdf->Cell($widths[2], 8, $row['product'], 1);
    $pdf->Cell($widths[3], 8, number_format($row['price'], 2), 1, 0, 'R');
    $pdf->Cell($widths[4], 8, $row['quantity'], 1, 0, 'R');
    $pdf->Cell($widths[5], 8, number_format($row['bill_amount'], 2), 1, 0, 'R');
    $pdf->Cell($widths[6], 8, number_format($row['amount_received'], 2), 1, 0, 'R');
    $pdf->Cell($widths[7], 8, $row['entry_date'], 1);
    $pdf->Ln();
}
