...
$mainTableData = [];
$result = $connAccounting->query("SELECT transaction_id, customer_name, customer_id, product, price, quantity, bill_amount, amount_received, our_cost, channel, profit, margin, entry_date FROM main_table WHERE transaction_id IN ($transactionIDList)");
while ($row = $result->fetch_assoc()) {
    $mainTableData[] = $row;
}

$accNetworkData = [];
$result = $connAccounts->query("SELECT transaction_id, customer_id, account_holder, product_category, amount_used, amount_paid, entry_date, channel FROM acc_network_main WHERE transaction_id IN ($transactionIDList)");
while ($row = $result->fetch_assoc()) {
    $accNetworkData[] = $row;
}
