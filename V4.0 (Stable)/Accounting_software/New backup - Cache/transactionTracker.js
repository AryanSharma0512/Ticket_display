function trackTransaction() {
    ...
    .then(data => {
        if (data.error) {
            ...
            function generateTableRows(data, tableName, excludedFields) {
                let tableHTML = "";
                if (data && data.length > 0) {
                    tableHTML += `<h3>${tableName} Results:</h3><div class="transaction-table-container"><table border='1' class="transaction-table">`;
                    tableHTML += "<tr>";
            ...
            resultsHTML += generateTableRows(data.main_table, "Main Table", excludedMainFields);
            resultsHTML += generateTableRows(data.acc_network_main, "Accounts Network", excludedAccNetworkFields);
